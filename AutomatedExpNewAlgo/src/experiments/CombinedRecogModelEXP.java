...
public class CombinedRecogModelEXP {
			...
			if(validReq){
				...
				for(int m=0;m<attributes.length; m++){
					...
					for(int k=0;k<seeds_array.length;k++){
						...
						File results_filepath = new File("/home/priyanka/Documents/grounded_language_learning/AutomatedExpNewAlgo/CombinedRecogModelResults/" + global_attr + "/" + rc_modality);
	...
	public static void createCSVFile(String [] attributes) throws Exception{
		...
		String results_path = "/home/priyanka/Documents/grounded_language_learning/AutomatedExpNewAlgo/CombinedRecogModelResults/";

		for(int m=0;m<attributes.length;m++){
			String writeFilePath = results_path + "/" + attributes[m] + "/" + chooseContextToClusterFrom(attributes[m]) + "/PointsToPlot.csv";
			File folder = new File(results_path + "/" + attributes[m] + "/" + chooseContextToClusterFrom(attributes[m]));
			File[] listOfFiles = folder.listFiles();
				...
				for(int i=0;i<listOfFiles.length;i++){
					...
					if(!(listOfFiles[i].isDirectory())){
						if(!(fileName.equals("MaxNumOfInstances.txt")) && !(fileName.equals("MinNumOfInstances.txt")) && !(fileName.equals("PointsToPlot.csv"))){
							FileReader fileReader = new FileReader(results_path + "/" + attributes[m] + "/" + chooseContextToClusterFrom(attributes[m]) + "/" +  fileName);
						    BufferedReader bufferedReader = new BufferedReader(fileReader);
