...
public class ContextAndAttrMappingExp {
	...
	static String [] rc_behavior_modalities = {"drop_audio"};
			/*{"revolve_audio","push_audio", "hold_haptics", "lift_haptics",
	...
	static String[] attributes = {"material", "size", "has_contents", "weight", "height", "deformable", "color", "shape", "deformable_from_top"};
	...
	// Constructor
	public ContextAndAttrMappingExp(){
	...
	public static void main(String[] args) {
		...
		try{
			...
			if(validReq){
				...
				for (int j=0;j<rc_behavior_modalities.length; j++){
					...
					for(int m=0;m<attributes.length; m++){
						...
						for(int k=0;k<seeds_array.length;k++){
							...
							File results_filepath = new File("/home/priyanka/Documents/grounded_language_learning/AutomatedExpNewAlgo/ContextAttrMappingResults/" + rc_behavior_modalities[j] + "/"+global_attr);
				...
				findMaxInstances(rc_behavior_modalities, attributes);
				findUltimateMax(rc_behavior_modalities, attributes);
				System.out.println("Producing points to plot");
				createCSVFile(rc_behavior_modalities, attributes);
	...
	public static void buildClassifierAndTest(Pair curContextPair, String modality, PrintWriter writer){
		...
		try{
			...
			//labeling function labels trials by labels
			//System.out.println("Object list: " + objects_list.size() + " object labels: " + object_labels.size());
			IClassLabelFunction LF = new ClassLabelID(objects_list, object_labels);
			/*ArrayList<String> class_values = LF.getUniqueValueSet(objects_list);
			//System.out.println("Class values in build classifier: " + class_values.toString());
			String [] classVals = new String[class_values.size()];
			...
			System.out.println("Training objects: " + training_objs);
			...
			Remove remove2 = new Remove();
			remove2.setAttributeIndices(Integer.toString(test.numAttributes()-1));
			remove2.setInvertSelection(false);
			remove2.setInputFormat(test);
				
			Instances testing = weka.filters.Filter.useFilter(test, remove2);
			
			//System.out.println("Testing: " + testing.toString());
			
			/*for(int t=0;t<testing.numInstances(); t++){
				//System.out.println("BEFORE Test instance at " + t + ": " + test.instance(t));
				System.out.println("Test number of classes: " + test.instance(t).numClasses());
				System.out.println("Test distribution for instance: " + Arrays.toString(C_mb.distributionForInstance(test.instance(t))));
				EV.updateStatsForClassifier(C_mb.distributionForInstance(testing.instance(t)), testing.instance(t));
			}*/
			...
			writer.println("\t\tNEW ITERATION: Train on "+train.numInstances()+" and test on "+ testing.numInstances());
						
			EV.evaluateModel(C_mb, testing);
	...
	public static void createCSVFile(String [] rc_behavior_modalities, String [] attributes) throws Exception{
		// File path to store the results in 
		// USUAL PATH
		//String results_path = "/home/priyanka/Documents/grounded_language_learning/AutomatedExpNewAlgo/results_exp2/";
		...
		for(int g=0;g<rc_behavior_modalities.length;g++){
			for(int m=0;m<attributes.length;m++){
				String writeFilePath = results_path + rc_behavior_modalities[g] + "/" + attributes[m] + "/PointsToPlot.csv";
				File folder = new File(results_path + rc_behavior_modalities[g] + "/" + attributes[m]);
				File[] listOfFiles = folder.listFiles();
					...
					for(int i=0;i<listOfFiles.length;i++){
						...
						if(!(listOfFiles[i].isDirectory())){
							if(!(fileName.equals("MaxNumOfInstances.txt")) && !(fileName.equals("MinNumOfInstances.txt")) && !(fileName.equals("PointsToPlot.csv"))){
								FileReader fileReader = new FileReader(results_path + rc_behavior_modalities[g] +  "/" + attributes[m] + "/" + fileName);
							    BufferedReader bufferedReader = new BufferedReader(fileReader);
