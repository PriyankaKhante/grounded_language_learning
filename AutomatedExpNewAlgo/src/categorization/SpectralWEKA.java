 ...
 */
public class SpectralWEKA extends AbstractClusterer implements OptionHandler {
  /**
  ...
  protected Instances m_Sequences;
  
  ArrayList<String> seqList;
  
  protected double [][] sim_matrix;
  
  boolean partitionOnlyOnce = true;
  int numPartitions;
  ...
  /**
   * Returns the normalized dissimilarity degree (or cut) between two s
  ...
  public void setSimMatrix(double [][] sm){
	  sim_matrix = sm;
  }
  ...
  public int getClusterNumber(String objectID){
	  ...
	  for (int i = 0; i < m_Sequences.numInstances(); i ++){
		  if (objectID.equals(m_Sequences.instance(i).stringValue(0)))
			  datasetIndex = i;
	  }
