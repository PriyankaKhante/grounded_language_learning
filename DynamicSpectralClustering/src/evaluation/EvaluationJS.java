 ...
 */
public class EvaluationJS implements Summarizable {
  ...
  private boolean m_savePredictions;
   ...
   */
  public EvaluationJS(Instances data) throws Exception {
   ...
   */
  public EvaluationJS(int numClasses, String [] classValues, CostMatrix costMatrix) 
       throws Exception {

	m_savePredictions = false;	
  ...
  public EvaluationJS(Instances data, CostMatrix costMatrix) 
  throws Exception {
  ...
  public void setSavePredictions(boolean s){
	  m_savePredictions=s;
  }
  ...
  public void addPrediction( Instance instance, double [] dist){
	  if (m_Predictions == null) 
  ...
  public void setUniformPriors() throws Exception {

	  
	for (int i = 0; i < m_ClassPriors.length; i ++){
		m_ClassPriors[i] = 1.0/(double)m_ClassPriors.length;
	}
}
  ...
  public double getAccVariance(FastVector predictions, int num_samples, double sample_fraction){
	 
	  Random R = new Random(1);
	  
	  double [] rates = new double[num_samples];
	  
	  for (int s = 0; s < rates.length; s++){
		  FastVector subset = new FastVector();
		  for (int i = 0; i < predictions.size(); i ++){
			  if (R.nextDouble() < sample_fraction)
				  subset.addElement(predictions.elementAt(i));
		  }
		  try {
				EvaluationJS EV_s = new EvaluationJS(this.m_NumClasses, this.m_ClassNames, null);
				EV_s.computeStatisticsNominal(subset);
				rates[s]=EV_s.pctCorrect();
			
				//System.out.print("\t"+subset.size()+" "+rates[s]);
		  } catch (Exception e) {
	 ...
	 // System.out.println("mean = "+Utils.mean(rates));
	  
	  return Utils.variance(rates);
  }
  
  public double getAccVariance(FastVector predictions, int set_size){
	  int mod_k = (int)Math.ceil((double)predictions.size()/(double)set_size);
	  double [] rates = new double[mod_k];
	  //System.out.print("mod_k = "+mod_k);
  ...
  public void computeStatisticsNominal(FastVector predictions){
	  for (int i = 0; i < predictions.size(); i++){
		  NominalPrediction p_i=(NominalPrediction)predictions.elementAt(i);
		  try {
			this.updateStatsForClassifier(p_i.distribution(), (int)p_i.actual());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
  ...
  public void updateStatsForClassifier(double [] predictedDistribution,
	    ...
	    if (m_savePredictions == true){
		  	if (m_Predictions == null) {
				m_Predictions = new FastVector();
			}
		  
		  	m_Predictions.addElement(new NominalPrediction(actualClass, predictedDistribution, 
				    1.0));
	    }
		double costFactor = 1;
		...
		// prediction (-m_CostMatrix.getElement(actualClass,actualClass)),
		// although often this will be zero
				m_TotalCost += instanceWeight * m_CostMatrix.getMaxCost(actualClass);
			} else {
				m_TotalCost += instanceWeight * m_CostMatrix.getElement(actualClass, predictedClass);
			}
		}
		...
		m_SumSchemeEntropy -= Utils.log2(predictedProb) *instanceWeight;
		m_SumPriorEntropy -= Utils.log2(priorProb) *instanceWeight;
		
		updateNumericScores(predictedDistribution, 
			  makeDistribution((double)actualClass), 
			  instanceWeight);
		
		// Update other stats
		m_ConfusionMatrix[actualClass][predictedClass] += instanceWeight;
		if (predictedClass != actualClass) {
			m_Incorrect += instanceWeight;
		} else {
			m_Correct += instanceWeight;
		}
   ...
   */
  public void updateNumericScores(double [] predicted, 
